...
const demoUser: User = {
  id: 1,
  name: "Maya Green",
  email: "hello@spendwise.demo",
  role: "USER",
  emailVerified: true,
};

const categories: Category[] = [
  { id: 1, name: "Groceries", color: "#6f956f" },
  { id: 2, name: "Home", color: "#bd7656" },
  { id: 3, name: "Wellbeing", color: "#b18a50" },
  { id: 4, name: "Travel", color: "#608b9a" },
  { id: 5, name: "Work", color: "#776b9c" },
];
...
function write<T>(key: string, value: T) {
  ...
  window.dispatchEvent(new Event("spendwise-change"));
}
...
export function signIn(email: string) {
  ...
  if (!user) {
    throw new Error("Those details do not match an account.");
  }
...
export const getCategories = () =>
  read<Category[]>(KEYS.categories, categories);

export const saveCategories = (items: Category[]) =>
  write(KEYS.categories, items);

export const getTransactions = () =>
  read<Transaction[]>(KEYS.transactions, transactions).sort((a, b) =>
...
export function addTransaction(item: Omit<Transaction, "id">) {
  saveTransactions([
    ...getTransactions(),
    {
      ...item,
...
export function removeTransaction(id: number) {
  saveTransactions(
    getTransactions().filter((item) => item.id !== id)
  );
}